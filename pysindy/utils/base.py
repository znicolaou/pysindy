...
#can we define our own integration function that assumes linear between grid points and uses the analytic integrals of the weights?

def old_convert_u_dot_integral(u, weak_pde_library):
    """
    ...
    for j in range(u.shape[-1]):
        for k in range(K):
            ...
            u_dot_integral[k, j] = integrate(-u_new*weight, weak_pde_library.xt_tilde_k[k]*weak_pde_library.H_xt, weak_pde_library.H_xt)
...
def convert_u_dot_integral(u, weak_pde_library):
    ...
    for axis in range(gdim):
        ...
        s[axis]=slice(None)
        s[-1]=axis
        grids=grids+[weak_pde_library.spatiotemporal_grid[tuple(s)]]

    dims=np.array(weak_pde_library.spatiotemporal_grid.shape)
    dims[-1]=u.shape[-1]
    x_interpolator=RegularGridInterpolator(grids, np.reshape(u,dims))

    for j in range(u.shape[-1]):
        for k in range(K):
            u_dot_integral[k, j] = integrate2(-x_interpolator(weak_pde_library.XT_interp_k[k])[...,j], weak_pde_library.XT_interp_k[k],weak_pde_library.domain_centers[k], weak_pde_library.H_xt, deriv_orders, weak_pde_library)
...
def integrate2(f, xt, xc, H_xt, derivs, weak_pde_lib):
    func_temp=f
    ndim=len(H_xt)
    ...
    for i in range(ndim):
        ...
        s[i]=slice(None,None,None)
        s[-1]=i
        weights=linear_weights((xt[tuple(s)]-xc[i])/H_xt[i],derivs[i],weak_pde_lib.p)*H_xt[i]**(1-derivs[i])
